...
export default function DownloadButton() {
  ...
  const animationEnd = () => {
    console.log("oi");
    downloadLoader.current.setAttribute(`class`, "hidden");
    downloadCheckMark.current.removeAttribute(`class`);
    downloadText.current.innerHTML = "Baixado";
    setTimeout(() => {
      downloadText.current.innerHTML = "Currículo";
      downloadCheckMark.current.setAttribute(`class`, "hidden");
      downloadIcon.current.removeAttribute(`class`);
    }, [1000]);
  };

  const animationStart = () => {
    downloadIcon.current.setAttribute(`class`, "hidden");
    downloadLoader.current.removeAttribute(`class`);
